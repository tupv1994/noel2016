...
var bodies, elements, text, text2;
...
function randomHopQua() {
    var _tu = 1;
    var _den = 53;
    return 'hopqua/img' + Math.floor(Math.random() * (_den - _tu + 1) + _tu) + '.png';
}
...
function createBall(x, y) {
    ...
    objImg.onSelectStart = null;

    objImg.setAttribute('src', _src);
    objImg.setAttribute('height', size);
    objImg.setAttribute('width', size);
    ...
    objImg.style.left = ((size - objImg.clientWidth) / 2) + 'px';
    objImg.style.top = ((size - objImg.clientHeight) / 2) + 'px';

    var _msg = msgs[Math.random() * msgs.length >> 0];
    objImg.ondblclick = function () { ShowMsg(_msg, _src); };
...
function loop() {
    ...
    for (i = 0; i < bodies.length; i++) {
        ...
        if (element.tagName == 'DIV') {
            ...
            element.style.transform = rotationStyle;
